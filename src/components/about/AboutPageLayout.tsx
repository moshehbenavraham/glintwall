                  ...
                  <div key={index}>
                    ...
                    <ul className="space-y-2">
                      {clientCategory.clients.map((client, clientIndex) => (
                        <li
                          key={clientIndex}
                          // `text-gray-400` isn't registered as a theme token
                          // (only -50/-100/-200/-300/-500/-700/-900 are mapped
                          // to CSS vars), so it would resolve to Tailwind's
                          // hardcoded light-gray and never adapt to dark mode.
                          // Use the semantic muted-foreground token instead.
                          className="text-sm sm:text-base lg:text-[1.0625rem] leading-snug text-gray-700 pl-4 relative before:content-['•'] before:absolute before:left-0 before:text-muted-foreground"
                        >
                          {client}
                        </li>
                      ))}
